...
class HfpWarningViewModel : IApaWarningViewModel, KoinComponent {
    ...
    private val dialogBox: LiveData<ApaDialogBox> =
        Transformations.map(apaRepository.warningMessage) { warningId ->
            val label = apaViewModelMapper.toHfpLabelMessage(warningId)
            ...
            if (label != null && warningId != WARNING_NONE) {
                ...
                when (warningId) {
                    ...
                    // Following messages are either unused or FAPK only
